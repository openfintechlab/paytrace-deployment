...
ALTER TABLE oftl_fwcsv_row_dispatch
    DROP CONSTRAINT IF EXISTS chk_oftl_fwcsv_row_dispatch_status;
...
ALTER TABLE oftl_fwcsv_registry
    DROP CONSTRAINT IF EXISTS chk_oftl_fwcsv_registry_status;

ALTER TABLE oftl_fwcsv_registry
    ADD CONSTRAINT chk_oftl_fwcsv_registry_status
    CHECK (status IN ('COMPLETED', 'FAILED', 'PROCESSED', 'PROCESSING', 'RESP_FILE_GENERATED', 'READY_FOR_RESPONSE'));

-- [MFB:20260606]: Added new response-related statuses to the status check constraint on oftl_fwcsv_registry.
ALTER TABLE oftl_fwcsv_registry
    DROP CONSTRAINT IF EXISTS chk_oftl_fwcsv_registry_response_status;

ALTER TABLE oftl_fwcsv_registry
    ADD CONSTRAINT chk_oftl_fwcsv_registry_response_status
